...
class SocketClient {
  ...
  connect(url: string, path: string, token: string): Socket<SocketEvents, SocketEvents> {
    console.log("================================");
    ...
    this.socket = io(url, {
      ...
      transports: ["websocket"], // Force WebSocket for NGINX Ingress
      path, // Use provided path (e.g., "/collaboration/socket.io")
      auth: {
        token,
      },
    });
